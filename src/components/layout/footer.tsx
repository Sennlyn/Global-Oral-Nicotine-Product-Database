...
export function Footer() {
  ...
  return <footer className="site-footer"><div className="container"><div className="footer-main"><div><Link href="/" className="footer-logo">GONPD<span>.</span></Link><p className="footer-tagline">{tr(locale,"A structured reference for the evolving landscape of oral nicotine and smokeless tobacco.")}</p></div><div className="footer-links"><div><strong>{tr(locale,"Explore")}</strong><Link href="/products">{tr(locale,"Products")}</Link><Link href="/categories">{tr(locale,"Categories")}</Link><Link href="/formats">{tr(locale,"Formats")}</Link></div><div><strong>{tr(locale,"Database")}</strong><Link href="/brands">{tr(locale,"Brands")}</Link><Link href="/markets">{tr(locale,"Markets")}</Link><Link href="/compare">{tr(locale,"Compare")}</Link></div><div><strong>{tr(locale,"Project")}</strong><Link href="/about">{tr(locale,"About")}</Link><span>{tr(locale,"Version 1.5 · Product database")}</span></div></div></div><div className="footer-disclaimer"><strong>{tr(locale,"Research use only")}</strong><p>{tr(locale,disclaimer)}</p></div><div className="footer-bottom"><span>© {new Date().getFullYear()} {locale === "zh" ? "全球口腔尼古丁产品数据库" : "Global Oral Nicotine Product Database"}</span><span>{tr(locale,"Independent research database · source-linked product records")}</span></div></div></footer>;
}
