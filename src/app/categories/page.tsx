...
export default async function CategoriesPage() { const locale=await getLocale(); return <div className="container page-shell"><Breadcrumb items={[{label:"Categories"}]}/><div className="page-heading"><p className="eyebrow">{tr(locale,"PRODUCT TAXONOMY / 01")}</p><h1>{tr(locale,"Product categories")}<span className="heading-period">.</span></h1><p>{tr(locale,"Six broad product classes cover oral nicotine and oral smokeless tobacco. Physical formats and subcategories remain separate research dimensions.")}</p></div><div className="taxonomy-intro"><span>{categories.length} {tr(locale,"CATEGORIES")}</span><p>{tr(locale,"Browse a category for typical formats, delivery routes, technical parameters and future verified product records.")}</p></div><div className="category-grid">{categories.map((category,index)=><CategoryCard key={category.id} category={category} index={index}/>)}</div></div>; }
