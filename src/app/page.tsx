...
export default function Home() {
  ...
  return <>
    ...
    <section className="section container"><SectionHeader eyebrow="03 / GEOGRAPHY" title="Global markets" description="A regional framework for future country level research. No market product data has been entered." href="/markets" linkLabel="Explore markets"/><div className="region-grid">{regions.map((region, index) => <Link className="region-card" href={`/markets/${region.slug}`} key={region.slug}><span className="region-num">{String(index+1).padStart(2,"0")}</span><div><strong>{region[locale]}</strong></div><ArrowUpRight size={18}/></Link>)}</div></section>
    <section className="section section-dark"><div className="container"><SectionHeader eyebrow="04 / METHODOLOGY" title="Built around evidence." description="The catalog is designed to make research claims inspectable, comparable and updateable."/><div className="principle-grid">{principles.map((item) => <div className="principle" key={item.title}><item.icon size={24} strokeWidth={1.5}/><h3>{tr(locale,item.title)}</h3><p>{tr(locale,item.text)}</p></div>)}</div><div className="framework-callout"><div><span className="callout-tag">{tr(locale,"CURRENT PHASE")}</span><strong>{tr(locale,"Database structure first. Evidence backed records next.")}</strong><p>{tr(locale,"The V1 framework contains taxonomy, model, search and comparison interfaces. Product data is intentionally empty.")}</p></div><Link href="/about" className="button button-light">{tr(locale,"About this database")} <ArrowUpRight size={18}/></Link></div></div></section>
  </>;
}
